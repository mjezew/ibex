...
jqueryWidget: {
    _init: function () {
        ...
        for (var i = 0; i < this.orderedAnswers.length-1; ++i) {
            var li;
	...
	if (this.orderedAnswers[7]){
                var fl = $("<table" + (conf_centerItems ? " align='center'" : "") + ">");
                        fl.append(this.orderedAnswers[7]);
                        fl.addClass(this.cssPrefix + "fake-link");
		this.element.append(fl);
		(function(i) {
                fl.click(function () { __Question_callback2__(i); });
            })(i);
		__Question_answers__[i] = ans;
            	__Question_callback2__ = function (i) {
                var answerTime = new Date().getTime();
		soundManager.stopAll();
                var ans = __Question_answers__[i];
                var correct = "NULL";
